...
public class StudentRegistrationAction extends DialogEsc implements
	...
	private static final String STATE[] = StateDetails.getInstance()
			.getStatesArray();
	...
	public StudentRegistrationAction(StudentDetails studentDetails,
			StudentFeeDetails studentFeeDetails, String title, int i) {
			...
			if (studentFeeDetails != null) {
				...
				try {
					List<StudentFeeDetails> list = HiebernateStudentDboUtil
							.getStudentFeeDetails(studentDetails);
					this.studentFeeDetails = list.size() > 0 ? list.get(0)
							: null;
				} catch (HibernateException e) {
					Logger.DEBUG.log(Level.WARNING,
							"Database Error while getting Student Fee details");
		...
		if (studentDetails != null) {
			try {
				...
				Logger.DEBUG.log(Level.WARNING,
						"Database Error while getting Student Fee details");
				Logger.EXCEPTION.log(Level.WARNING,
						"Database Error while getting Student Fee details", e);
			}
			setRegisterButtonEnabled(false);
			setData(studentDetails);
			setTabEnabled(true);
	...
	private JPanel tabbedPanel() {
		...
		StudentImagePanel imagePanel = new StudentImagePanel(
				studentDetails == null ? null
						: studentDetails.getStudentImageDetails(), this);
			...
			public void stateChanged(ChangeEvent arg0) {
				if (studentDetails == null
					...
					MsgDialogUtils.showWarningDialog(
							StudentRegistrationAction.this,
							"First Register Student Details");
	...
	private JPanel getRegistraionPanel() {
		...
		c.gridwidth = 2;
		c.anchor = GridBagConstraints.LINE_START;
		txtMotherName = new JTextField(12);
	...
	public void actionPerformed(ActionEvent e) {
		if (OperationConstant.STUDENT_REGISTER.equals(e.getActionCommand())) {
			...
			try {
				...
				this.studentDetails = HiebernateStudentDboUtil
				...
				Logger.EXCEPTION.log(Level.WARNING,
						"Error Occured while save student Details", exp);
				return;
			}
			if (result) {
				ActionDispatcher.fireAction(OperationConstant.STUDENT_REGISTER,
						studentDetails);
				MsgDialogUtils.showInformationDialog(this,
						"Student Details has been registered Successfully!");
				Logger.DEBUG
						.info("Student Details has been registered Successfully");
				setTabEnabled(true);
			...
			try {
				...
				MsgDialogUtils.showWarningDialog(this,
						"Database error occured! ");
				Logger.EXCEPTION.log(Level.WARNING,
						"Error Occured while updating student Details", exp);
				return;
			}
			if (result) {
				ActionDispatcher.fireAction(OperationConstant.STUDENT_UPDATE,
						studentDetails);
				MsgDialogUtils.showInformationDialog(this,
	...
	private void setData(StudentDetails student) {
		...
		txtDOB.setText(ApplicationConstant.sdf_date.format(student
				.getDateOfBirth()));
		...
		cbSession.setSelectedItem(student.getSessionDetails()
				.getDisplayString());

	}

	private StudentDetails createStudentObject(StudentDetails student)
		...
		student.setClassDetails(classComboBoxModel.getSelectedObject(cbClass
				.getSelectedIndex()));
		student.setSessionDetails(sessionComboBoxModel
				.getSelectedObject(cbSession.getSelectedIndex()));
		student.setDateOfBirth(ApplicationConstant.sdf_date.parse(txtDOB
				.getText().trim()));
		student.setParentsOccupation(cbParentOccupation.getSelectedItem()
				.toString());
		...
		student.setRegistrationNumber(Utils.generateRegistrationNumber(
				student.getSessionDetails(), student.getClassDetails(),
	...
	private String validateInput() {
		...
		if ("".equals(txtFatherName.getText().trim())
				&& "".equals(txtMotherName.getText().trim())) {
			error = true;
		...
		if ("".equals(txtPhone.getText().trim())) {
			...
			sb.append("\n");
		} else if (!ValidationUtils.isValidPhone(txtPhone.getText().trim())) {
			error = true;
			sb.append("Please Enter Valid Phone Number format and start with 0 or +91 ");
			sb.append("\n");
		}
		if (!"".equals(txtEmailId.getText().trim())
				&& !ValidationUtils.isValidEmailAddress(txtEmailId.getText()
						.trim())) {
			error = true;
	...
	public void propertyChange(PropertyChangeEvent evt) {
		if (DISPOSE_DIALOG.equals(evt.getPropertyName())) {
			...
			StudentImageDetails studentImageDetails = (StudentImageDetails) evt
					.getNewValue();
